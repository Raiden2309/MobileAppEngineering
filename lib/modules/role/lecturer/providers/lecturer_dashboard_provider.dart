            ...
            for (var doc in classesSnapshot.docs) {
              ...
              mappedClasses.add(
                ClassModel(
                  ...
                  name: name,
                  code:
                      d['subjectCode']?.toString() ??
                      d['classCode']?.toString() ??
                      'COMP000',
            ...
            _enrollmentsSubscription = _db
                ...
                .listen((enrollmentSnapshot) {
                  ...
                  for (var classItem in classes) {
                    final classDocs = enrollmentSnapshot.docs.where(
                      (doc) =>
                    ...
                    for (var cDoc in classDocs) {
                      ...
                      final double bo = (cData['burnoutIndex'] as num? ?? 0.0)
                          .toDouble();
                      if (bo >= 0.70) classAtRisk++;

                      final double comp = (cData['completedTasks'] as num? ?? 0)
                          .toDouble();
                      final double pend = (cData['pendingTasks'] as num? ?? 0)
                          .toDouble();
                      if ((comp + pend) > 0) {
                    ...
                    classItem.avgCompletion = classProgressCount > 0
                        ? (classProgressSum / classProgressCount) * 100
                        : 0.0;
                  }
                  ...
                  for (var doc in enrollmentSnapshot.docs) {
                    ...
                    final double burnout =
                        (studentData['burnoutIndex'] as num? ?? 0.0).toDouble();
                    final double completed =
                        (studentData['completedTasks'] as num? ?? 0).toDouble();
                    final double pending =
                        (studentData['pendingTasks'] as num? ?? 0).toDouble();
                    ...
                    if (burnout >= 0.70) {
                      ...
                      alerts.add(
                        AlertModel(
                          ...
                          message:
                              '${studentData['studentName'] ?? 'A student'} is exhibiting high exhaustion metrics.',
                          type: 'burnout',
                          time: 'Just now',
                          emoji: '🔥',
                          meta:
                              ...
                              'Class Route',
                          studentId:
                              studentData['studentId']?.toString() ?? doc.id,
                          studentName:
                              studentData['studentName']?.toString() ??
                              'Student',
                          className:
                              studentData['classId']?.toString() ?? 'Class',
                  ...
                  double averageCompletion = progressCount > 0
                      ? (combinedProgress / progressCount) * 100
                      : 0.0;
                  ...
                  _stats = [
                    ...
                    StatCardModel(
                      ...
                      sub: highlyBurnedOutCount > 0
                          ? 'Requires attention'
                          : 'All clear',
                      icon: Icons.warning_amber_rounded,
                      accent: highlyBurnedOutCount > 0
                          ? Colors.red
                          : Colors.grey,
                    ),
                  ];
