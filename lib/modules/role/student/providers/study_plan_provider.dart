...
class StudyPlanProvider with ChangeNotifier {
  ...
  void updateSettingsProvider(StudentSettingsProvider p) =>
      _settingsProvider = p;
  ...
  void _generateLocalFastPlan() {
    ...
    final int availableWindowMins = (studyEndMins - studyStartMins).clamp(
      0,
      14 * 60,
    );
    ...
    final Map<int, int> dayNextStart = {
      for (int i = 0; i < 7; i++) i: studyStartMins,
    };
    final Map<int, List<StudyBlock>> dayBlocks = {
      for (int i = 0; i < 7; i++) i: [],
    };
    ...
    for (final slot in blockedSlots) {
      ...
      final int dayIdx = dayNames.indexOf(parts[0].toLowerCase());
    ...
    final sortedTasks = List<Map<String, dynamic>>.from(_latestFirestoreTasks)
      ..sort((a, b) {
        if (a['status'] == BlockStatus.inProgress &&
            b['status'] != BlockStatus.inProgress) {
          return -1;
        }
        if (b['status'] == BlockStatus.inProgress &&
            a['status'] != BlockStatus.inProgress) {
          return 1;
        }
    ...
    for (final task in sortedTasks) {
      ...
      while (remainingMinutes >= minSessionMinutes &&
          dayCursor < eligibleDayIndexes.length) {
        final int dayIdx = eligibleDayIndexes[dayCursor];
        final int budgetLeft =
            dayBudgetMinutes[dayIdx]! - dayUsedMinutes[dayIdx]!;
        ...
        dayCursor++;
        if (dayCursor >= eligibleDayIndexes.length &&
            remainingMinutes >= minSessionMinutes) {
          dayCursor = 0;
          final anyBudget = eligibleDayIndexes.any(
            (idx) =>
                (dayBudgetMinutes[idx]! - dayUsedMinutes[idx]!) >=
                minSessionMinutes,
          );
          if (!anyBudget) break;
        }
    ...
    if (subjects.isNotEmpty) {
      ...
      for (int i = 0; i < 7; i++) {
        ...
        while (true) {
          ...
          final int sessionMins = budgetLeft.clamp(
            minSessionMinutes,
            maxSessionMinutes,
          );
          ...
          // Add break if room
          final int budgetAfterRevision =
              dayBudgetMinutes[i]! - dayUsedMinutes[i]!;
    ...
    final List<DayPlan> generatedDays = List.generate(7, (i) {
      ...
      final blocks = dayBlocks[i]!
        ..sort((a, b) {
          final aMins =
              int.parse(a.startTime.split(':')[0]) * 60 +
              int.parse(a.startTime.split(':')[1]);
          final bMins =
              int.parse(b.startTime.split(':')[0]) * 60 +
  ...
  Future<void> _saveTasksToCache() async {
    ...
    if (uid == null) return;
    try {
      final serializable = _latestFirestoreTasks
          .map(
            (t) => {
              'title': t['title'],
              'subject': t['subject'],
  ...
  Future<bool> _loadTasksFromCache() async {
    ...
    try {
      ...
      final List<dynamic> list = decoded is String
          ? jsonDecode(decoded)
          : List.from(decoded);
  ...
  void listenToLiveStudyPlan({String? semester}) {
    ...
    if (_enrollmentSubscription != null &&
        _subscribedUid == uid &&
        _subscribedSemester == semester) {
      return;
    }
    _subscribedUid = uid;
    _subscribedSemester = semester;
          ...
          (snapshot) {
            ...
            for (var doc in snapshot.docs) {
              ...
              final String className =
                  dataMap['classId']?.toString() ?? 'General';
  ...
  void reset() {
    ...
    _subscribedUid = null;
    _subscribedSemester = null;
    plan = null;
    _latestFirestoreTasks = [];
    loading = false;
    isOffline = false;
    error = null;
    notifyListeners();
  ...
  Future<void> generateAiWeeklyPlan() async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> taskInput = _latestFirestoreTasks.map((
        t,
      ) {
      ...
      final String studyStart = settings != null
          ? toHHMM(settings.studyStart)
          : '08:00';
      final String studyEnd = settings != null
          ? toHHMM(settings.studyEnd)
          : '22:00';

      final List<String> blockedSlots = settings?.blockedSlots.toList() ?? [];
      final List<String> enrolledSubjects = <String>{
        ...?settings?.joinedClasses.map((c) => c.name),
        ...?settings?.subjects
            .map((s) => s['name'] ?? '')
            .where((n) => n.isNotEmpty),
      ...
      for (var dayMap in (aiResponse['days'] ?? [])) {
        ...
        for (var blockMap in (dayMap['blocks'] ?? [])) {
          ...
          if (blockMap['status'] == 'inProgress') {
            bStatus = BlockStatus.inProgress;
          }
          if (blockMap['status'] == 'completed') {
            bStatus = BlockStatus.completed;
          }
          ...
          blocks.add(
            StudyBlock(
              ...
              durationMinutes: (blockMap['duration_minutes'] as num? ?? 60)
                  .toInt(),
              type: bType,
              status: bStatus,
            ),
