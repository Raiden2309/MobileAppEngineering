...
class StudentDashboardProvider with ChangeNotifier {
  ...
  void listenToLiveDashboardStats() {
    ...
    // Avoid churning the Firestore subscription on repeated invocations
    // (e.g. from task mutations). The live snapshot refreshes automatically
    // and the per-snapshot semester filter picks up semester switches.
    if (_enrollmentSubscription != null && _subscribedUid == uid) return;
    _subscribedUid = uid;
            ...
            data = DashboardModel(
              ...
              stats: DashboardStats(
                ...
                totalWeeks: _semesterEnd != null && _semesterStart != null
                    ? (_semesterEnd!.difference(_semesterStart!).inDays / 7)
                          .ceil()
                    : 14,
              ),
  ...
  void reset() {
    ...
    isLoading = false;
    isOffline = false;
    _activeScheduleTask = "Free Time / Break";
        ...
        .map((snapshot) {
          ...
          for (var doc in snapshot.docs) {
            ...
            final String subjectCode =
                d['subjectCode']?.toString() ?? d['code']?.toString() ?? '';
            final String displayLabel = subjectCode.isNotEmpty
                ? subjectCode
                : className;
            ...
            for (var t in tasks) {
              ...
              final String? scheduledDateStr =
                  t['scheduledDate']?.toString() ??
                  t['scheduled_date']?.toString();
              final String estHours =
                  (t['estimated_hours'] ?? t['estimatedHours'] ?? '1.0')
                      .toString();
              ...
              final String? targetDateParsingSource =
                  scheduledDateStr ?? dueDateStr;

              if (targetDateParsingSource != null) {
                final cleanDateSegment = targetDateParsingSource
                    .split('T')
                    .first;
